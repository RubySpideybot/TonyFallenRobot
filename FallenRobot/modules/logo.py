...
async def lego(event):
 ...
 if event.sender_id != OWNER_ID and not quew:
  await event.reply('`Give some text to create a logo baby!`\n`Example /logo <ⲩⲟⲟⲛⲓⲉ>`')
  return
 ...
 try:
    ...
    fname = "fallen.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Logo generated by ⲩⲟⲟⲛⲓⲉ")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Floodwait Error, report this at @Horny_RUBY')


__mod_name__ = "Logo"

__help__ = """
*ⲩⲟⲟⲛⲓⲉ* can create some beautiful and attractive logo for your profile pics.
